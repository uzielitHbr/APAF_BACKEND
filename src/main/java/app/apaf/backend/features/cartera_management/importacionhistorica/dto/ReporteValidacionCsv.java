    ...
    public void addError(int lineNumber, String field, String message) {
        if (errores.size() >= maxErrores) {
            throw new CampoCsvInvalidoException(
                    "Se alcanzó el límite máximo de errores (" + maxErrores + "). Último error: " + message);
        }
    ...
    public void throwIfInvalid() {
        if (hasErrors()) {
            throw new CampoCsvInvalidoException(String.format(
                    "El archivo contiene %d errores bloqueantes y %d advertencias. Primer error: Línea %d, Campo %s - %s",
                    errores.size(), advertencias.size(), errores.get(0).getLineNumber(), errores.get(0).getField(),
                    errores.get(0).getMessage()));
        }
    }
}
